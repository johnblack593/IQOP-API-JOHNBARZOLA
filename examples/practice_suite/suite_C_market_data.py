...
def run(api: IQ_Option, collector: ReportCollector) -> None:
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "C-01: Open time retrieval", True, duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "C-01: Open time retrieval", False, detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "C-02: Active asset resolution", True, detail=status_msg, duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "C-02: Active asset resolution", False, detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "C-03: Instruments retrieval", True, detail=f"Found {len(instruments['instruments'])} crypto instruments", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "C-03: Instruments retrieval", False, detail=str(e), duration=time.time() - start))
    ...
    try:
        if hasattr(api, "get_digital_underlying_list_data"):
            ...
            collector.record(TestResult(SUITE_NAME, "C-04: Digital strikes", True, detail=f"Found {len(strikes)} assets", duration=time.time() - start))
        else:
            collector.record(TestResult(SUITE_NAME, "C-04: Digital strikes", False, detail="Method get_digital_underlying_list_data not found", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "C-04: Digital strikes", False, detail=str(e), duration=time.time() - start))
