...
def run(api: IQ_Option, collector: ReportCollector) -> None:
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "H-01: Forced WS disconnect", "PASSED", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "H-01: Forced WS disconnect", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        import tests.practice_suite.config as config
        ...
        collector.record(TestResult(SUITE_NAME, "H-02: Reconnect and auth", "PASSED", detail=f"Balance: {bal}", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "H-02: Reconnect and auth", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "H-03: Backoff reset", "PASSED", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "H-03: Backoff reset", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "H-04: Post-reconnect mode", "PASSED", detail=f"Mode: {mode}", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "H-04: Post-reconnect mode", "FAILED", detail=str(e), duration=time.time() - start))
