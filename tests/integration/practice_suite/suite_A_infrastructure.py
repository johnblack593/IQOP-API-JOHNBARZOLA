...
def run(api: IQ_Option, collector: ReportCollector) -> None:
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "A-01: Version check", "PASSED", detail=f"Version: {ver}", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "A-01: Version check", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        try:
            ...
            pass # Expected state
        collector.record(TestResult(SUITE_NAME, "A-02: Rate limiter token avail", "PASSED", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "A-02: Rate limiter token avail", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "A-03: TLS session singleton", "PASSED", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "A-03: TLS session singleton", "FAILED", detail=str(e), duration=time.time() - start))
    ...
    try:
        ...
        collector.record(TestResult(SUITE_NAME, "A-04: Reconnect manager reset", "PASSED", duration=time.time() - start))
    except Exception as e:
        collector.record(TestResult(SUITE_NAME, "A-04: Reconnect manager reset", "FAILED", detail=str(e), duration=time.time() - start))
