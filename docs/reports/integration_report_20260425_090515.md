...
**Fecha:** 2026-04-25 09:08:08 UTC  
**Duración total:** 172.3s  
...
|------|--------|---------|--------|
| import:IQ_Option (stable_api) | ✅ PASS | OK | 0.44s |
...
| import:Asset Scanner | ✅ PASS | OK | 0.12s |
...
| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| connect() | ✅ PASS | Conectado: None | 8.01s |
| change_balance(PRACTICE) | ✅ PASS | Balance PRACTICE: $9940.84 | 2.5s |
...
| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| get_all_open_time() | ✅ PASS | Tipos: ['binary', 'turbo', 'cfd', 'forex', 'crypto', 'digital'] \| Abiertos: 606 | 127.09s |
| ACTIVES KYC IDs | ✅ PASS | ACTIVES: 638 entradas \| KYC IDs verificados | 0.0s |
| get_all_profit() | ✅ PASS | Profits OK (284 activos) \| Muestra: [('GBPJPY', defaultdict(<class 'dict'>, {'tu | 0.76s |
| binary/turbo assets open | ✅ PASS | Binary/Turbo abiertos: 293 \| Primeros: ['ARBUSD-OTC', 'UK100-OTC', 'XRPUSD-OTC'] | 0.0s |
| digital assets open | ✅ PASS | Digital abiertos: 156 \| Primeros: ['META/GOOGLE-OTC', 'BONKUSD-OTC', 'EURNZD-OTC | 0.0s |
...
| offset_seconds() razonable | ✅ PASS | Offset: 0.3742s | 3.0s |
| server_now() coherente | ✅ PASS | server.now() − time.time() = 0.3742s | 0.0s |
...
| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| buy() Turbo 1M | ✅ PASS | Turbo buy OK: asset=EURUSD-OTC order=13835710599 | 0.25s |
| check_win_v3() Turbo result | ❌ FAIL | check_win_v3() retornó None — timeout o trade no cerró | 0.02s |
| buy() Binary 1M | ✅ PASS | Binary buy OK: asset=EURUSD-OTC order=13835710605 | 0.25s |
...
| clock offset post-Blitz | ✅ PASS | Clock offset post-Blitz: 0.3058s ✓ | 0.0s |

### ⚠️ L7 (1/2)

| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| Smart ID generator | ✅ PASS | Smart ID generado: do2107A20260425D090738T5MCSPT | 0.0s |
| buy_digital_spot() 5M | ❌ FAIL | buy_digital_spot() falló para EURUSD-OTC — id={'code': 'error_place_digital_orde | 0.29s |

### ✅ L8 (3/3)

| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| get_candles() histórico | ✅ PASS | Candles OK: 10 velas de 1M para EURUSD-OTC | 0.22s |
| get_server_timestamp() | ✅ PASS | server_timestamp=1777108058.419 \| diff local=0.469s | 0.0s |
| start/stop candles stream | ✅ PASS | Realtime price EURUSD-OTC: defaultdict(<class 'dict'>, {}) | 23.17s |
