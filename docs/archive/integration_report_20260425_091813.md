...
**Fecha:** 2026-04-25 09:20:46 UTC  
**Duración total:** 153.0s  
...
|------|--------|---------|--------|
| import:IQ_Option (stable_api) | ✅ PASS | OK | 0.22s |
...
| import:Asset Scanner | ✅ PASS | OK | 0.18s |
...
| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| connect() | ✅ PASS | Conectado: None | 7.88s |
| change_balance(PRACTICE) | ✅ PASS | Balance PRACTICE: $9942.33 | 2.57s |
...
| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| get_all_open_time() | ✅ PASS | Tipos: ['binary', 'turbo', 'cfd', 'forex', 'crypto', 'digital'] \| Abiertos: 627 | 130.21s |
...
| binary/turbo assets open | ✅ PASS | Binary/Turbo abiertos: 302 \| Primeros: ['TAOUSD-OTC', 'FETUSD-OTC', 'US100/JP225 | 0.0s |
| digital assets open | ✅ PASS | Digital abiertos: 165 \| Primeros: ['FWONA-OTC', 'USDJPY-OTC', 'AUS200-OTC'] | 0.0s |
...
| offset_seconds() razonable | ✅ PASS | Offset: 0.3821s | 3.0s |
| server_now() coherente | ✅ PASS | server.now() − time.time() = 0.3821s | 0.0s |
...
| buy() Turbo 1M | 💥 ERROR | Traceback (most recent call last):
  File "D:\Programacion\API-IQ\IQOP-API-JOHNB | 0.01s |
...
| clock offset post-Blitz | ✅ PASS | Clock offset post-Blitz: 0.3821s ✓ | 0.0s |

### ⚠️ L7 (1/2)

| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| Smart ID generator | ✅ PASS | Smart ID generado: do2107A20260425D092038T5MCSPT | 0.0s |
| buy_digital_spot() 5M | ❌ FAIL | buy_digital_spot() falló para EURUSD-OTC — id={'code': 'error_place_digital_orde | 2.27s |

### ⚠️ L8 (1/3)

| Test | Status | Detalle | Tiempo |
|------|--------|---------|--------|
| get_candles() histórico | ❌ FAIL | get_candles() retornó vacío para EURUSD-OTC | 0.4s |
| get_server_timestamp() | ✅ PASS | server_timestamp=1777108840.676 \| diff local=0.261s | 0.0s |
